...
const CONST = {
  ...
  sendDelay: 8,
  // MQTT 브로커
  mqttBroker: 'mqtt://192.168.0.120:1883',
  // MQTT 수신 Delay(ms)
  mqttDelay: 1000*4,
...
//    {deviceId: 'Thermo', subId: '5', stateHex: Buffer.alloc(4,'b07c0500','hex'), power: 'off', setTemp: '', curTemp: ''}
    {deviceId: 'gas', subId: '1', stateHex: Buffer.alloc(4,'ad56017a','hex'), valve: 'OFF'}, //밸브 off
    {deviceId: 'gas', subId: '1', stateHex: Buffer.alloc(4,'ad56007b','hex'), valve: 'ON'}, //밸브 on
    {deviceId: 'ev', subId: '1', stateHex: Buffer.alloc(4,'ad2f0002','hex'), call: 'ON'}, //엘리베이터콜 on
    {deviceId: 'ev', subId: '1', stateHex: Buffer.alloc(4,'b0410071','hex'), call: 'OFF'}, //엘리베이터콜 off
  ],
  ...
  DEVICE_COMMAND: [
    ...
    {deviceId: 'gas', subId: '1', commandHex: Buffer.alloc(4,'ab780053','hex'), valve: 'OFF'}, //밸브--off
    {deviceId: 'ev', subId: '1', commandHex: Buffer.alloc(4,'b02f011e','hex'), call: 'ON'}, //엘리베이터콜 on
  ],
...
parser.on('data', function (data) {
	...
	switch (data[1]) {
		case 0x56: 	// 가스, ev 상태 정보
		 ...
		 case 0x2f:
			...
			data.copy(ack3, 0, 1, 2);
			var objFoundIdx3 = queue.findIndex(obj => obj.commandHex.includes(ack3));
			if(objFoundIdx3 > -1) {
				log('[Serial] Success command:', data.toString('hex'));
				queue.splice(objFoundIdx3, 1);
			}
			break;
	}}
	...
	switch (data[1]) {
		...
		case 0x7a: case 0x7d: case 0x7f: case 0x4f:
			// Ack 메시지를 받은 명령은 제어 성공하였으므로 큐에서 삭제
			const ack2 = Buffer.alloc(3);
			data.copy(ack2, 0, 1, 4);
		...
		case 0x78:
			...
			data.copy(ack, 0, 1, 3);
			var objFoundIdx2 = queue.findIndex(obj => obj.commandHex.includes(ack));
			if(objFoundIdx2 > -1) {
				log('[Serial] Success command:', data.toString('hex'));
				queue.splice(objFoundIdx2, 1);
			}
			break;	
	}
